...
DEFAULT_DYNAMIC_LEARNING_PATH = (
    "ai_dynamic_learning_rinandlen.json"  # New file for dynamic learning examples
)
...
class AICog(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.memory_file_path = os.getenv(
            "BOT_MEMORY_PATH", DEFAULT_MEMORY_PATH
        ...
        self.manual_context_file_path = os.getenv(
            "BOT_MANUAL_CONTEXT_PATH", DEFAULT_MANUAL_CONTEXT_PATH
        )
        self.load_manual_context()  # Load manual context
        self.dynamic_learning_file_path = os.getenv(
            "BOT_DYNAMIC_LEARNING_PATH", DEFAULT_DYNAMIC_LEARNING_PATH
        )
    ...
    **Rules:**  
        ...
        try:
            ...
            if not os.path.exists(memory_dir):
                ...
                try:
                    ...
                    print(
                        f"FATAL: Could not create memory directory {memory_dir}. Memory will not persist. Error: {e}"
                    )
            ...
            if os.path.exists(self.memory_file_path):
                ...
                print(
                    f"Loaded memory for {len(self.user_memory)} users from {self.memory_file_path}"
                )
            else:
                print(
                    f"Memory file not found at {self.memory_file_path}. Starting with empty memory."
                )
                self.user_memory = {}
        except json.JSONDecodeError as e:
            print(
                f"Error decoding JSON from memory file {self.memory_file_path}: {e}. Starting with empty memory."
            )
            self.user_memory = {}
        except Exception as e:
            print(
                f"Error loading memory from {self.memory_file_path}: {e}. Starting with empty memory."
            )
            self.user_memory = {}
        ...
        try:
            ...
            if not os.path.exists(memory_dir):
                try:
                    ...
                    print(
                        f"ERROR: Could not create memory directory {memory_dir} during save. Save failed. Error: {e}"
                    )
    ...
    def add_user_fact(self, user_id: str, fact: str):
        ...
        if not any(
            ...
            for existing_fact in self.user_memory[user_id_str]
        ):
    ...
    def load_history(self):
        ...
        try:
            if os.path.exists(self.history_file_path):
                ...
                print(
                    f"Loaded conversation history for {len(self.conversation_history)} users from {self.history_file_path}"
                )
            else:
                print(
                    f"History file not found at {self.history_file_path}. Creating empty file."
                )
                self.conversation_history = {}
                self.save_history()  # Create the file immediately
        except json.JSONDecodeError as e:
            print(
                f"Error decoding JSON from history file {self.history_file_path}: {e}. Starting with empty history."
            )
            self.conversation_history = {}
        except Exception as e:
            print(
                f"Error loading history from {self.history_file_path}: {e}. Starting with empty history."
            )
            self.conversation_history = {}
    ...
    def load_manual_context(self):
        ...
        try:
            if os.path.exists(self.manual_context_file_path):
                ...
                print(
                    f"Loaded {len(self.manual_context)} manual context entries from {self.manual_context_file_path}"
                )
            else:
                print(
                    f"Manual context file not found at {self.manual_context_file_path}. Creating empty file."
                )
                self.manual_context = []
                self.save_manual_context()  # Create the file immediately
        except json.JSONDecodeError as e:
            print(
                f"Error decoding JSON from manual context file {self.manual_context_file_path}: {e}. Starting empty."
            )
            self.manual_context = []
        except Exception as e:
            print(
                f"Error loading manual context from {self.manual_context_file_path}: {e}. Starting empty."
            )
            self.manual_context = []
    ...
    def save_manual_context(self):
        ...
        try:
            ...
            print(
                f"Error saving manual context to {self.manual_context_file_path}: {e}"
            )
    ...
    def load_dynamic_learning(self):
        ...
        try:
            if os.path.exists(self.dynamic_learning_file_path):
                ...
                print(
                    f"Loaded {len(self.dynamic_learning)} dynamic learning entries from {self.dynamic_learning_file_path}"
                )
            else:
                print(
                    f"Dynamic learning file not found at {self.dynamic_learning_file_path}. Creating empty file."
                )
                self.dynamic_learning = []
                self.save_dynamic_learning()  # Create the file immediately
        except json.JSONDecodeError as e:
            print(
                f"Error decoding JSON from dynamic learning file {self.dynamic_learning_file_path}: {e}. Starting empty."
            )
            self.dynamic_learning = []
        except Exception as e:
            print(
                f"Error loading dynamic learning from {self.dynamic_learning_file_path}: {e}. Starting empty."
            )
            self.dynamic_learning = []
    ...
    def save_dynamic_learning(self):
        ...
        try:
            ...
            print(
                f"Error saving dynamic learning to {self.dynamic_learning_file_path}: {e}"
            )
    ...
    async def generate_response(
        ...
        if not self.api_key:
            return (
                "Sorry, the AI API key is not configured. I cannot generate a response."
            )

        guild_id = (
            source_message.guild.id
            if source_message and source_message.guild
            else (
                source_interaction.guild.id
                if source_interaction and source_interaction.guild
                else None
            )
        )
        channel_id = (
            ...
            if source_message
                ...
                source_interaction.channel.id
                if source_interaction and source_interaction.channel
                else None
            )
        )
        channel = (
        ...
        timeout_match = re.search(
            r"timeout\s+<@!?(\d+)>(?:\s+for\s+(\d+)\s*(minute|minutes|min|mins|hour|hours|day|days))?",
            prompt,
            re.IGNORECASE,
        )
        search_match = re.search(
            r"search(?:\s+for)?\s+(.+?)(?:\s+on\s+the\s+internet)?$",
            prompt,
            re.IGNORECASE,
        )
        ...
        if self.manual_context:
            manual_context_str = "\n".join(
                [f"- {item}" for item in self.manual_context]
            )
        else:
        ...
        if self.dynamic_learning:
            dynamic_learning_str = "\n".join(
                [f"- {item}" for item in self.dynamic_learning]
            )
        else:
                    ...
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            if not data.get("choices") or not data["choices"][0].get(
                                ...
                                print(
                                    f"API Error: Unexpected response format. Data: {data}"
                                )
                            ...
                            if (
                                ...
                                print(
                                    f"AI requested tool calls: {response_message['tool_calls']}"
                                )
                                ...
                                for tool_call in tool_calls:
                                    ...
                                    try:
                                        ...
                                        if function_name == "run_safe_shell_command":
                                            ...
                                            if command_to_run:
                                                if self.is_safe_command(command_to_run):
                                                    print(
                                                        f"Executing safe command: '{command_to_run}'"
                                                    )
                                                    tool_result_content = (
                                                    ...
                                                    print(
                                                        f"Blocked unsafe command: '{command_to_run}'"
                                                    )
                                            ...
                                            function_name == "remember_fact_about_user"
                                        ):
                                            ...
                                            if (
                                                fact_user_id == user_id_str
                                                and fact_to_remember
                                            ):
                                                self.add_user_fact(
                                                    fact_user_id, fact_to_remember
                                                )
                                                ...
                                                not fact_user_id or not fact_to_remember
                                            ):
                                        ...
                                        print(
                                            f"Error decoding tool arguments: {tool_call.get('function', {}).get('arguments')}"
                                        )
                                        messages.append(
                                            {
                                                "role": "tool",
                            ...
                            # No tool calls, or finished after tool calls
                                ...
                                print(
                                    f"AI Response for {user_name}: {final_response[:100]}..."
                                )  # Log snippet

                                # --- Add interaction to history ---
                                self.add_to_history(
                                    user_id_str, "user", f"{user_name}: {prompt}"
                                )  # Add user prompt
                                self.add_to_history(
                                    user_id_str, "assistant", final_response
                                )  # Add AI response
                                # ----------------------------------
                                ...
                                print(
                                    f"API Error: No content and no tool calls in response. Data: {data}"
                                )
    ...
    def is_safe_command(self, command: str) -> bool:
        ...
        if (
            cmd_name == "ip"
            and len(parts) > 1
            and parts[1].lower() in ["addr", "link", "route"]
        ):
        ...
        for i, part in enumerate(parts):
            ...
            for char in dangerous_chars:
                if char in part:
                    print(
                        f"Unsafe command blocked (dangerous char '{char}' in '{part}'): {command}"
                    )
                    return False
            ...
            if cmd_name == "ping" and i > 0:
                ...
                # Allow the target hostname/IP
                    ...
                    print(
                        f"Unsafe command blocked (invalid ping argument '{part}'): {command}"
                    )
                    return False
    ...
    async def run_shell_command(self, command: str) -> str:
        ...
        try:
            ...
            process = await asyncio.create_subprocess_shell(
                ...
                limit=1024
                * 100,  # Limit buffer size (e.g., 100KB) to prevent memory issues
            )
            ...
            return (
                f"Error: Command not found or invalid command: '{command.split()[0]}'"
            )
        except Exception as e:
    ...
    async def timeout_user(self, guild_id: int, user_id: int, minutes: int) -> bool:
        ...
        try:
            ...
            await member.timeout(
                duration, reason=f"Timed out by Kasane Teto via AI command"
            )
            return True
        except Exception as e:
    ...
    async def search_internet(self, query: str) -> str:
        ...
        if not serp_api_key:
            return "Search is disabled (missing API key)."
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, timeout=15.0) as response:
                    if response.status == 200:
                        ...
                        if summary:
                            results.append(
                                f"**Summary:** {(summary[:300] + '...') if len(summary) > 300 else summary}"
                            )
                        if not summary and data.get("knowledge_graph"):
                            ...
                            if title and desc:
                                ...
                                results.append(
                                    f"**Info:** {(kg_text[:350] + '...') if len(kg_text) > 350 else kg_text}"
                                )
                            if kg.get("source", {}) and kg.get("source", {}).get(
                                "link"
                            ):
    ...
    async def check_admin_permissions(self, interaction: discord.Interaction) -> bool:
        ...
        await interaction.followup.send(
            "Hehe, you need **Administrator** powers for this! ✨", ephemeral=True
        )
        return False
    ...
    async def slash_ai(self, interaction: discord.Interaction, prompt: str):
        ...
        try:
            response = await self.generate_response(
                user_id, user_name, prompt, source_interaction=interaction
            )
            # Split long messages
            if len(response) > 2000:
                for chunk in [
                    ...
                    )  # Suppress embeds for chunks
            else:
                await interaction.followup.send(response, suppress_embeds=True)
        except Exception as e:
            print(f"Error in slash_ai: {e}")
            await interaction.followup.send(
                f"A critical error occurred processing that request. Please tell my developer! Error: {type(e).__name__}"
            )

    @app_commands.command(
        name="aiconfig", description="Configure AI settings (Admin Only)"
    )
    ...
    async def slash_aiconfig(
        ...
        user_id = str(
            interaction.user.id
        )  # Still configures the *admin's* personal settings
        ...
        config_message = (
            ...
            + "\n".join(
                [f"- {k.replace('_',' ').title()}: `{v}`" for k, v in config.items()]
            )
            + "\n\nChanges:\n- "
            + "\n- ".join(changes)
        )
    ...
    @app_commands.command(
        name="context", description="Add a piece of context for the AI (Admin Only)"
    )
    ...
    async def slash_context(self, interaction: discord.Interaction, text: str):
        ...
        if self.add_manual_context(text):
            await interaction.followup.send(
                f"Okay~! Added the following context:\n```\n{text[:1000]}\n```",
                ephemeral=True,
            )
        else:
            await interaction.followup.send(
                "Hmm, I couldn't add that context. Maybe it was empty or already exists?",
                ephemeral=True,
            )

    @app_commands.command(
        name="addlearning",
        description="Add a dynamic learning example for the AI (Admin Only)",
    )
    ...
    async def slash_addlearning(self, interaction: discord.Interaction, text: str):
        ...
        if self.add_dynamic_learning(text):
            await interaction.followup.send(
                f"Okay~! Added the following learning example:\n```\n{text[:1000]}\n```",
                ephemeral=True,
            )
        else:
            await interaction.followup.send(
                "Hmm, I couldn't add that learning example. Maybe it was empty or already exists?",
                ephemeral=True,
            )

    @app_commands.command(
        name="aichannel",
        description="Toggle Teto responding to *all* messages here (Admin Only)",
    )
    async def slash_aichannel(self, interaction: discord.Interaction):
        ...
        if not interaction.channel:
            await interaction.followup.send("Cannot use here.")
            return
        channel_id = interaction.channel.id
        if channel_id in self.active_channels:
            self.active_channels.remove(channel_id)
            await interaction.followup.send(
                f"Okay! I won't reply to *every* message in {interaction.channel.mention} anymore. 😊"
            )
        else:
            self.active_channels.add(channel_id)
            await interaction.followup.send(
                f"Yay! 🎉 I'll now respond to **all** messages in {interaction.channel.mention}!"
            )
    ...
    async def on_message(self, message: discord.Message):
        ...
        )  # Keep channel_id check for active_channels
        ...
        if (
            re.match(mention_pattern, message.content)
            ...
            rf"\b{re.escape(self.bot.user.name)}\b", message.content, re.IGNORECASE
        ):
            should_respond = True
        ...
        if should_respond and prompt and self.api_key:
            ...
            async with message.channel.typing():
                try:
                    response = await self.generate_response(
                        user_id, user_name, prompt, source_message=message
                    )
                    reply_func = (
                        message.reply
                    ...
                    if len(final_response) > 2000:
                        first_chunk = True
                        for chunk in [
                            final_response[i : i + 1990]
                            for i in range(0, len(final_response), 1990)
                        ]:
                            send_func = (
                                reply_func if first_chunk else message.channel.send
                            )
            ...
            if random.random() < reaction_chance:
                ...
                teto_emojis = [
                    ...
                    "🎉",
                    "👍",
                    "<:teto_smile:123456789>",
                    "<:teto_wink:123456789>",
                ]  # Add custom emoji IDs if available
...
async def setup(bot: commands.Bot):
    ...
    manual_context_path = os.getenv(
        "BOT_MANUAL_CONTEXT_PATH", DEFAULT_MANUAL_CONTEXT_PATH
    )
    dynamic_learning_path = os.getenv(
        "BOT_DYNAMIC_LEARNING_PATH", DEFAULT_DYNAMIC_LEARNING_PATH
    )
    ...
    if not ai_api_key:
        print(
            "!!! WARNING: AI_API_KEY not set. AI features WILL NOT WORK. Ensure it contains your OpenRouter key. !!!"
        )
    else:
        print(
            f"AI_API_KEY loaded (ends with ...{ai_api_key[-4:]}). Using OpenRouter API."
